...
### Completed Recently
- **BLOCK 59: Bug Fix — Variant/Color Not Showing on Hive Cards** - DONE (2026-03-10). Root cause: existing records in DB had no `color_variant` stored. Fix: (1) `build_post_response` resolves `color_variant` from record when post doesn't have it. (2) Composer endpoints now store `color_variant` on post docs. (3) Background backfill task on startup fetches variant from Discogs API for all records missing it (backfilled 113/140 records). (4) Manual `/api/backfill-variants` endpoint for per-user trigger. Files: `hive.py`, `collection.py`, `server.py`, `PostCards.js`.
