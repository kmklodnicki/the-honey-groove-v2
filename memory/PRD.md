...
- **Bug Fix: Dream Wishlist Value vs ISO Value** - DONE (2026-03-10). Separated the combined `/valuation/wishlist` endpoint into two independent calculations: `/valuation/wishlist` now returns only Actively Seeking (OPEN) items, and new `/valuation/dreamlist` returns only Dream Wishlist (WISHLIST) items. Both have public `/{username}` variants. Frontend updated: CollectionPage DreamDebtHeader now uses dreamlist endpoint (shows "Dream Wishlist Value"), toggle button still uses wishlist endpoint (shows "Value of ISOs"). ProfilePage now displays both values separately with distinct labels and styling. Files: `backend/routes/valuation.py`, `frontend/src/pages/CollectionPage.js`, `frontend/src/pages/ProfilePage.js`.
