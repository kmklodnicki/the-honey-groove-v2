...
128. **Demo/Test Content Permanent Filter** — Found 7 of 8 Explore endpoints were completely missing the hidden_ids filter (only /buzzing had it). Added get_hidden_user_ids() + filter to ALL 8: trending, trending posts, most-wanted, near-you, recent-hauls, suggested-collectors, active-isos, buzzing. Also added hard username/email filter on near-you and suggested-collectors queries. get_hidden_user_ids() excludes: is_hidden=true, is_test=true, username=demo/test*, email=*@example.com/*@test.com. Cleaned 1 orphaned spin + 2 orphaned notifications. (Mar 2026)

129. **Image Proxy Endpoint** — Created GET /api/files/serve/{path:path} public proxy endpoint to serve uploaded images from Emergent storage. Includes namespace security check (403 for paths outside honeygroove/), correct Content-Type headers. Upload endpoint now returns {file_id, path, url}. (Mar 2026)

130. **Profile Photo Upload Fix** — Root cause: SettingsPage was constructing URLs as `integrations.emergentagent.com/storage/public/{path}` which returns 404 (no such public URL exists). Fixed: frontend now constructs URL as `${API}/files/serve/${path}` which routes through the proxy endpoint. Full chain verified end-to-end: upload → proxy serves image → save to user DB → re-fetch returns new URL. (Mar 2026)

131. **Listing Photo Upload Fix** — Root cause: ISOPage was storing raw storage paths (e.g. `honeygroove/uploads/xxx/yyy.jpg`) as photo_urls, which aren't valid URLs. Fixed: now constructs URL as `${API}/files/serve/${path}`. (Mar 2026)
...
166. **Notification Click Navigation** — Each notification type now navigates to its correct destination on click. Expanded NOTIF_ICONS map with all types. (Mar 2026)
167. **Comment Replies + @Mentions** — Nested replies (parent_id), @username autocomplete (GET /api/mention-search), MENTION + COMMENT_REPLY notifications. CommentItem.js extracted as separate component. (Mar 2026)
168. **Comment Likes** — Heart/like toggle on comments (POST/DELETE /api/comments/{id}/like), COMMENT_LIKED notification. (Mar 2026)
