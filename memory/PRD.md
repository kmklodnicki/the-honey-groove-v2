...
### Completed Recently
- **BLOCK 59: Bug Fix — Variant/Color Not Showing on Hive Cards** - DONE (2026-03-10). Root cause: `build_post_response` only read `post.get("color_variant")` (always None for NOW_SPINNING), and composer endpoints never stored `color_variant` on post docs. Fix: (1) `build_post_response` now resolves `color_variant` from record when post doesn't have it. (2) `composer_now_spinning` stores `color_variant` from record on the post doc. (3) `add_record` stores `color_variant` on ADDED_TO_COLLECTION post doc. (4) Frontend prioritizes `post.color_variant` (resolved by backend) over `record.color_variant`. Files: `hive.py`, `collection.py`, `PostCards.js`.
