...
- **Bug Fix: Dream Wishlist Value vs ISO Value** - DONE (2026-03-10). Separated the combined `/valuation/wishlist` endpoint into two independent calculations: `/valuation/wishlist` now returns only Actively Seeking (OPEN) items, and new `/valuation/dreamlist` returns only Dream Wishlist (WISHLIST) items. Both have public `/{username}` variants. Renamed all UI labels from "Value of ISOs" to "Value of Dream Records" across CollectionPage (toggle, DreamDebtHeader, subtract animation), ProfilePage (stats display, empty state, taste match), and removed redundant ISO value display. Dream value toggle now uses `/valuation/dreamlist` exclusively. Files: `backend/routes/valuation.py`, `frontend/src/pages/CollectionPage.js`, `frontend/src/pages/ProfilePage.js`.
