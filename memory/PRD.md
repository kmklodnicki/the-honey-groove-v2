...
- **Global Label Update: "Wishlist" → "Dream List"** - DONE (2026-03-10). Replaced all user-facing instances of "Wishlist", "Wantlist", "Dreaming" labels with "Dream List" across the entire platform. Updated: CollectionPage (tab label, toggle button, DreamDebtHeader, subtitle, subtract animation, empty states), ProfilePage (tab label, stats, empty states, taste match text), ExplorePage + ExploreSeeAllPage (toast messages, empty states), AddRecordPage (title, toast), FAQPage (entire "Wantlist" section → "Dream List"), SettingsPage (delete account warning), MessagesPage (empty state), WaxReportPage (section titles), ListingDetailModal (button labels, toasts), VariantActions (button labels, toasts), Weekly Wax email (fallback text, section copy), and email templates (onboarding, wantlist match). Internal code (function names, status constants, API routes) left as-is per spec. Fixed bug: `setWishlistValue` reference in CollectionPage replaced with `setDreamlistValue`. All lint checks pass.
