...
async def build_post_response(post: Dict, current_user_id: Optional[str] = None):
    ...
    if post_user:
        _spin_count = await db.spins.count_documents({"user_id": post_user["id"]})
        user_data = {"id": post_user["id"], "username": post_user["username"], "avatar_url": post_user.get("avatar_url"), "founding_member": post_user.get("founding_member", False), "title_label": post_user.get("title_label"), "golden_hive": post_user.get("golden_hive", False), "golden_hive_verified": post_user.get("golden_hive_verified", False), "is_verified": bool(post_user.get("isVerified", post_user.get("golden_hive_verified", False))), "spin_count": _spin_count}
    else:
        user_data = None
