  ...
  return (
    <ShareCardBase ref={ref} bg={bg} user={user}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: '100%', position: 'relative', zIndex: 1 }}>
        {/* "Daily Prompt" tag */}
        {/* opacity:0 preserves layout; Canvas 2D API draws the full pill (background + border + text). */}
        <div
          style={{ background: 'rgba(200,134,26,0.12)', border: '1.5px solid rgba(200,134,26,0.3)', borderRadius: 100, padding: '10px 32px', display: 'inline-block', textAlign: 'center', opacity: 0 }}
          data-canvas-pill="daily-prompt"
        >
