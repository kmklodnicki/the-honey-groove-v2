...
const FaqItem = ({ q, a }) => {
  ...
  return (
    ...
    >
      <button
        className="w-full flex items-center justify-between px-5 py-4 text-left text-[#1E2A3A] font-medium hover:bg-[#FFF8EE] transition-colors"
        onClick={() => setOpen((v) => !v)}
      >
        <span>{q}</span>
        {open ? <ChevronUp className="w-4 h-4 text-[#D4A828] shrink-0" /> : <ChevronDown className="w-4 h-4 text-[#D4A828] shrink-0" />}
      </button>
      {open && (
        <div className="px-5 pb-4 text-sm text-[#3A4D63]">{a}</div>
      )}
    </div>
  );
};
  ...
  return (
    <div className="max-w-[680px] mx-auto px-4 py-8 pb-24" data-testid="gold-page">
      ...
      <div className="text-center mb-10">
        <h1
          ...
          style={{
            ...
            background: 'linear-gradient(135deg, #D4A828 0%, #DAA520 50%, #E8A820 100%)',
        ...
        <p className="text-[16px] text-[#3A4D63]" style={{ fontFamily: 'Georgia, serif' }}>
      ...
      <div className="max-w-sm mx-auto border-2 border-[#DAA520] rounded-2xl p-8 text-center mb-8 bg-gradient-to-br from-[#FFF9E6] to-[#FFF3E0]" data-testid="gold-pricing-card">
        <p
          className="text-[42px] font-bold text-[#1E2A3A] mb-1"
          style={{ fontFamily: "'Playfair Display', Georgia, serif" }}
        >
          $4.99
        </p>
        <p className="text-sm text-[#3A4D63] mb-6">per month</p>
        <ul className="text-left space-y-3 mb-6">
          {[
            ...
            <li key={item} className="flex items-start gap-2 text-sm text-[#1E2A3A]">
        ...
        <Button
          className="w-full rounded-full font-bold text-[#1E2A3A] mb-2"
        ...
        <p className="text-xs text-[#3A4D63]">Cancel anytime.</p>
      </div>

      {/* FAQ */}
      <div className="space-y-3" data-testid="gold-faq">
        <h2
          className="font-bold text-[#1E2A3A] text-lg mb-4"
          style={{ fontFamily: "'Playfair Display', Georgia, serif" }}
        >
          Questions
        </h2>
