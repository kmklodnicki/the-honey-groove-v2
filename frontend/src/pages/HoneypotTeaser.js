  ...
  return (
    <div className="max-w-[680px] mx-auto px-4 py-6 pb-24" data-testid="honeypot-teaser">
      ...
      <div className="text-center py-10 bg-[#FFF8EE] rounded-2xl border border-[#F5E6CC] mb-6 px-6">
        <div className="inline-flex items-center justify-center mb-4">
          ...
          >
            ...
            <path
              ...
              opacity="0.9"
            />
            <ellipse cx="32" cy="20" rx="10" ry="6" fill="#D4A828" opacity="0.7" />
        ...
        <h1
          className="text-[28px] md:text-[32px] font-bold text-[#1E2A3A] mb-3 leading-tight"
          style={{ fontFamily: "'Playfair Display', Georgia, serif" }}
          data-testid="teaser-headline"
        >
          The Honeypot is almost ready.
        </h1>
        <p className="text-[15px] text-[#3A4D63] max-w-md mx-auto" style={{ fontFamily: 'Georgia, serif' }}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        {[
          ...
          >
            <Icon className="w-6 h-6 text-[#D4A828] mb-3" />
            <p className="font-bold text-[#1E2A3A] mb-1">{title}</p>
            <p className="text-sm text-[#3A4D63]">{body}</p>
          </div>
        ))}
      </div>
      ...
      <div className="bg-white border border-[#F5E6CC] rounded-xl p-6 mb-6">
        <h2 className="font-bold text-[#1E2A3A] mb-4 text-base" style={{ fontFamily: "'Playfair Display', Georgia, serif" }}>
          ...
          {feeBars.map(({ label, width, color, pct, amber }, i) => (
            <div key={i} className="flex items-center gap-3">
              <span
                ...
                style={amber ? { color: '#D4A828', fontWeight: 600 } : { color: '#3A4D63' }}
              >
                {label}
              </span>
              <div className="flex-1 h-8 bg-[#F3EBE0] rounded-lg overflow-hidden">
                <div
              ...
              <span className="text-sm font-medium text-[#1E2A3A] w-12 text-right shrink-0">{pct}</span>
            </div>
          ))}
        </div>
        <p className="text-xs text-[#3A4D63] mt-3 italic">Gold members pay just 4%.</p>
      </div>
      ...
      >
        <h2
          className="font-bold text-[#1E2A3A] text-lg mb-1"
          style={{ fontFamily: "'Playfair Display', Georgia, serif" }}
        >
        ...
        <Button
          ...
          className="rounded-full font-bold text-[#1E2A3A]"
          style={{ background: '#E8A820' }}
          data-testid="teaser-gold-btn"
        >
          Get Gold
        </Button>
        <p className="text-xs text-[#3A4D63] mt-2">$4.99/month. Cancel anytime.</p>
      </div>
      ...
      <div className="text-center mb-6" data-testid="teaser-notify-section">
        {user?.honeypot_notify_me ? (
          ...
          <Button
            ...
            style={{ borderColor: '#D4A828', color: '#D4A828', background: 'transparent' }}
        ...
        {!user?.honeypot_notify_me && (
          <p className="text-xs text-[#3A4D63] mt-2">We'll email you the moment the Honeypot goes live.</p>
        )}
      </div>
      ...
      <div className="bg-[#FFF8EE] border border-[#F5E6CC] rounded-xl p-6" data-testid="teaser-coming-list">
        <h2
          className="font-bold text-[#1E2A3A] mb-4 text-base"
          style={{ fontFamily: "'Playfair Display', Georgia, serif" }}
        >
          What's coming.
        </h2>
        <ul className="space-y-2">
          {comingItems.map((item, i) => (
            <li key={i} className="flex items-start gap-2 text-sm text-[#1E2A3A]">
              <CheckCircle2 className="w-4 h-4 text-[#D4A828] mt-0.5 shrink-0" />
              {item}
            </li>
          ))}
        </ul>
